...
def enter_creation_page():
    ...
    Parameters include: number of names, temperature, gender, and target.
    ...
    with st.form("names_creation", clear_on_submit=True):
        ...
        container.markdown(CONTAINER_MARKDOWN, unsafe_allow_html=True)
        ...
        creativity = creativity_col.slider(
            "Creativity Level",
            min_value=CREATIVITY_DICT["display_min"],
            max_value=CREATIVITY_DICT["display_max"],
            step=CREATIVITY_DICT["display_step"],
            label_visibility="hidden",
        )
        temperature = process_temperature(creativity)
